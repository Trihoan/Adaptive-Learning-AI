...
class AIService:
    ...
    def get_recommendation(self, math_score=None, prog_score=None, study_hours=None, video_rate=None, weak_topics=None):
        try:
            ...
            # Xử lý weak_topics để đưa vào lời khuyên
            weak_msg = ""
            topics_str = ""
            if weak_topics:
                topics_str = ", ".join(weak_topics[:2])
                weak_msg = f" Tập trung xem lại: {topics_str}."
            ...
            recommendations = {
                "Giỏi": {
                    ...
                    "action": [
                        ...
                        f"Tài liệu nâng cao: {peer_suggestion}"
                    ],
                    "next_step": "Duy trì cường độ để giữ vững điểm A+.",
                    "friendly_msg": f"Tuyệt vời! Kết quả {math_score}/10 cho thấy bạn đã nắm rất vững kiến thức.{weak_msg} Hãy thử làm thêm các đề tổng hợp 60 câu nhé!"
                },
                "Khá": {
                    "status": f"Đang tiến bộ rất tốt! Dự đoán kỳ thi: {is_passed}.",
                    "action": [
                        f"Tập trung tinh chỉnh lại kiến thức phần: {weak_topics[0] if weak_topics else 'Chương vừa làm'}",
                        f"Gợi ý tài liệu bổ sung: {peer_suggestion}"
                    ],
                    "next_step": "Cố gắng tăng thêm khoảng 30 phút học mỗi tuần để bứt phá lên nhóm Giỏi.",
                    "friendly_msg": f"Chào bạn, mình thấy bạn đang có nền tảng khá vững với mức điểm {math_score}.{weak_msg} Chỉ cần tập trung tinh chỉnh lại một chút, bạn hoàn toàn có thể chinh phục điểm số cao hơn!"
                },
                "Cần hỗ trợ": {
                    "status": f"Cần tập trung củng cố lại kiến thức! Dự đoán kỳ thi: {is_passed}.",
                    "action": [
                        f"Đọc kỹ giáo trình phần: {weak_topics[0] if weak_topics else 'Kiến thức nền tảng'}",
                        f"Xem lại video bài giảng tóm tắt: {topics_str if weak_topics else 'các chương vừa làm'}"
                    ],
                    "next_step": "Dành thêm ít nhất 1 giờ mỗi ngày để xem lại các khái niệm cơ bản.",
                    "friendly_msg": f"Đừng nản lòng nhé! Kết quả bài thi {math_score} điểm cho thấy bạn đang gặp chút khó khăn.{weak_msg} Mình khuyên bạn nên xem lại video tóm tắt và làm lại đề để củng cố nhé!"
                }
            }
